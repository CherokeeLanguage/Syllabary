...
public class XMPlayer extends Thread
{
    ...
    class Voice {
        ...
        //  Sleep(100);
            ...
            else if (volume_ > 64) volume_ = 64;

            // Clamp the panning_ voice
            if (panning_ < 0) panning_ = 0;
            else if (panning_ > 255) panning_ = 255;

            if (bNoteCutted_ == false) {
                vol = (fadeOutVolume_ / 65536.f) * (volume_ / 64.f) * (envelopeVolume_ / 64.f) * (globalVolume_ / 64.f) * mainVolume_;
        ...
        void portamentoTo(int speed)
        {
            if (period_ < dstPeriod_) {
                period_ += speed;
                if (period_ > dstPeriod_) period_ = dstPeriod_;
            }
        ...
        void seekEnvelope(int pos)
        {
            ...
            if ( (actuInstru_.volType & 1) != 0) {
                ...
                if (pos >= actuInstru_.envVol[ (nbPoints - 1) * 2]) {
                    ...
                    volEnvActuPos_ = (float) actuInstru_.envVol[ (nbPoints-1) * 2]-actuInstru_.envVol[ (nbPoints-2) * 2];
                    return;
                }
            ...
            if ( (actuInstru_.panType & 1) != 0) {
                ...
                if (pos >= actuInstru_.envPan[ (nbPoints - 1)*2]) {
                    ...
                    panEnvActuPos_ = (float)actuInstru_.envPan[ (nbPoints - 1)*2] - actuInstru_.envPan[ (nbPoints - 2) * 2];
                    return;
                }
    ...
    public OZMod.ERR load(PipeIn _input)
    {
        ...
        for (int i = 0; i < nbInstrus_; i++) {
           ...
           instru.samples = new Sample[ instru.nbSamples ];
            ...
            // and in a second pass _input.read the audio data (XM format really sucks!!)
            for (int j = 0; j < instru.nbSamples; j++) {
                ...
                // samples are stored as delta value, recompose them as absolute integer
                int nbBits = 0;
                if ( (sample.type & 16) != 0) {
                    // 16 bits
                    nbBits = 16;
                ...
                if ((sample.type & 3) == 1) {
                    // forward loop
                    sample.audioData.make(dat, nbBits, 1, (sample.startLoop) >> (nbBits/16), (sample.startLoop + sample.lengthLoop) >> (nbBits/16), AudioData.LOOP_FORWARD);
                }
                else if ( ((sample.type & 3) == 2) || ((sample.type & 3) == 3) ) {
                    // pingpong loop
                    sample.audioData.make(dat, nbBits, 1, (sample.startLoop) >> (nbBits/16), (sample.startLoop + sample.lengthLoop) >> (nbBits/16), AudioData.LOOP_PINGPONG);
                }
                else
    ...
    /**
     * Starts to play the XM. The time latency between a note is read and then heard is approximatively of 100ms.
     * If the XM is not loopable and finish, you cannot restart it by invoking again this method.
     */
    public void play()
    {
    ...
    {
        freq_ = 44100;
        AudioFormat format = new AudioFormat(freq_, 16, 2, true, true);

        try {
            line_ = AudioSystem.getSourceDataLine(format, OZMod.usedMixer_.getMixerInfo());
            line_.open(format, freq_);
      }
...
//                continue;
            ...
            if (voice.bGotArpeggio_ == true) voice.period_ = voice.periodBAK_;
  ...
  //                  sample = NULLSample_;
    //            else
            ...
            if (colum >= 0x10 && colum < 0x60) {
                ...
                if (colum != 0) voice.column_VolumeSlidingSpeed_ = -colum;
            }
                ...
                // Fine volume_ down
                colum -= 0x80;
                if (colum != 0) voice.column_FineVolumeDown_ = colum;
                ...
                // Fine volume_ up
                colum -= 0x90;
                if (colum != 0) voice.column_FineVolumeUp_ = colum;
                ...
                if (colum != 0) voice.column_PanSlidingSpeed_ = -colum;
            }
                ...
                if (colum != 0) voice.column_PortamentoSpeed_ = colum << 6;
            }
            ...
            case 0x1:
                // PORTAMENTO UP
                voice.effect_ = effect;
                if (effectOperand_ != 0) voice.portaSpeed_ = effectOperand_*4;
                break;
            ...
            case 0x3:
                ...
                if (note > 0 && note < 97 && voice.samplePlaying_ != null)
                    voice.dstPeriod_= getPeriod(note+voice.samplePlaying_.relativeNote, voice.fineTune_);
                voice.effect_ = effect;
                if (effectOperand_ != 0) voice.portamentoSpeed_ = effectOperand_*4;
                break;

            case 0x4:
                // VIBRATO
                voice.effect_ = effect;
                if ( (effectOperand_ & 0xf0) != 0) voice.vibratoSpeed_ = (effectOperand_ >> 4) * 4;
                if ( (effectOperand_ & 0x0f) != 0) voice.vibratoProf_ = effectOperand_ & 0x0f;
                break;
            ...
            case 0x5:
                ...
                if (note != 0 && voice.samplePlaying_ != null)
                    voice.dstPeriod_= getPeriod(note + voice.samplePlaying_.relativeNote, voice.fineTune_);
                voice.effect_ = effect;
                if ( (effectOperand_ & 0xf0) != 0) voice.volumeSlidingSpeed_ = effectOperand_ >>4 ;
                else if ( (effectOperand_ & 0x0f) != 0) voice.volumeSlidingSpeed_ = -(effectOperand_ & 0x0f);
                break;
            ...
            case 0x7:
                // TREMOLO
                voice.effect_ = effect;
                if ( (effectOperand_ & 0xf0) != 0) voice.tremoloSpeed_ = (effectOperand_>>4)*4;
                if ( (effectOperand_ & 0x0f) != 0) voice.tremoloProf_ = effectOperand_&0x0f;
                break;
            ...
            case 0x9:
                // SAMPLE JUMP
                voice.effect_ = effect;
                if (effectOperand_ != 0) voice.samplePosJump_ = effectOperand_<<8;
                break;
            ...
            case 0xa:
                ...
                voice.effect_ = effect;
                if ( (effectOperand_ & 0xf0) != 0) voice.volumeSlidingSpeed_ = effectOperand_ >> 4;
                else if ( (effectOperand_ & 0x0f) != 0) voice.volumeSlidingSpeed_ = -(effectOperand_ & 0xf);
                break;
            ...
            case 0xd:
                ...
                // Yes, posJump is given in BCD format. What is the interest ? Absolutely none, thanks XM..
                break;
            ...
            case 0xe:
                ...
                case 0x1:
                    // FineSlideUp
                    if (effectOperand2_ != 0) voice.fineSlideSpeed_ = effectOperand2_ * 4;
                ...
                case 0x6:
                    ...
                    if (effectOperand2_ != 0) {
                        ...
                        else bGotPatternLoop_ = false;
                    }
                    else patternPosLoop_ = posInPattern_;
                    break;
                case 0x7:
                    // Set Tremolo Form
                ...
                case 0xc:
                    // note Cut
                    voice.effect_ = 0xec;
                    if (effectOperand2_ != 0) voice.noteCut_ = effectOperand2_;
                    else voice.bNoteCutted_ = true;
                    break;
                case 0xd:
                    // note Delay
            ...
            case 0xf:
                // SET SPEED or BPM_
                if (effectOperand_ < 32) newSpeed = effectOperand_;
                else newBPM = effectOperand_;
                break;

            case 0xf+'g'-'f':
                // SET GLOBAL VOLUME
                globalVolume_ = effectOperand_;
                if (globalVolume_ > 64) globalVolume_ = 64;
                break;

            case 0xf+'h'-'f':
                // GLOBAL VOLUME SLIDING
                if (effectOperand_ != 0) voice.globalVolSlide_ = effectOperand_;
                if ( (voice.globalVolSlide_ & 0xf0) != 0) globalVolume_ += (voice.globalVolSlide_ >> 4) * 4;
                else if ( (voice.globalVolSlide_ & 0x0f) != 0) globalVolume_ -= (voice.globalVolSlide_ & 0xf) * 4;
                if (globalVolume_ > 64) globalVolume_ = 64;
                else if (globalVolume_ < 0) globalVolume_ = 0;
                break;

            case 0xf+'r'-'f':
                // MULTI RETRIG NOTE
                voice.bGotRetrigNote_ = true;
                if ( (effectOperand_ & 0x0f) != 0) voice.tickForRetrigNote_ = effectOperand_ & 0x0f;
                break;
        ...
        if ((posInPattern_ == patterns_[actuPattern].nbLines) || (bGotPatternJump == true) || (bGotPatternBreak == true)) {
    ...
    /**
     * Sets the XM loopable or not. The method can be called at any time if the song is still playing.
