...
public class S3MPlayer extends Thread
{
    ...
    class Voice {
        ...
        void updateSoundWithEffect()
        {
            ...
            case COMMAND_TREMOR:
                on = tremorValue_ >> 4; on++;
                off = tremorValue_ & 0xf; off++;
    ...
    public OZMod.ERR load(PipeIn _input)
    {
        ...
        for (int numInstru = 0; numInstru < nbInstrus_; numInstru++) {
            ...
            int b1 = doff[0]; int b2 = (doff[1] & 0xff) << 8; int b3 = (doff[2] & 0xff) << 16;
    ...
    int getST3period(int note, int c2speed)
    {
        ...
        if (c2speed == 0) return 1;
        else return h / c2speed;
    }
...
//    again:;
        ...
        if (actuPattern >= nbPatterns_) {
            ...
            if (posChanson_ == listLen_ || numListPattern_[ posChanson_ ] == 0xff) {
                sendEndNotify(notifiesList_, nbNotifies_);
                if (flags_ == FLAG_LOOP) {
                    posChanson_ = 0;
                    for (int i = 0; i < realNbChannels_; i++) {
                        voices_[i].sndchan_.stop();
                    }
                }
                else {
                    stop(true, false);
...
//            if (numVoice != 2)
  //              continue;
            ...
            if (voice.command_ != COMMAND_VIBRATO && voice.extraCommand_ != EXTRACOMMAND_DUALK && voice.bGotVibrato_ == true) {
            ...
            if ((voice.command_ == COMMAND_VIBRATO || voice.extraCommand_ == EXTRACOMMAND_DUALK) && voice.bGotVibrato_ == false) {
    ...
    /**
     * Starts to play the S3M. The time latency between a note is read and then heard is approximatively of 100ms.
     * If the S3M is not loopable and finish, you cannot restart it by invoking again this method.
     */
    public void play()
    {
    ...
    /**
     * Sets the S3M loopable or not. The method can be called at any time if the song is still playing.
